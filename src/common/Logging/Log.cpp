...
Log::Log() :
    ...
    dberLogfile(NULL), chatLogfile(NULL), sqlLogFile(NULL), sqlDevLogFile(NULL), miscLogFile(NULL), AntiCheatLogFile(NULL),
...
Log::~Log()
{
	...
	if (AntiCheatLogFile != NULL)
        fclose(AntiCheatLogFile);
    AntiCheatLogFile = NULL;
}
...
void Log::Initialize()
{
	...
	AntiCheatLogFile = openLogFile("Anticheat.LogFile", NULL, "a");
...
void Log::outAntiCheat(const char * str, ...)
{
    ...
    if (AntiCheatLogFile)
    {
        ...
        va_list ap;
        va_start(ap, str);
        vfprintf(AntiCheatLogFile, str, ap);
