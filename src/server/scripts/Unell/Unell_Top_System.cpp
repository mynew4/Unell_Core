...
void InitTopTotalTime()
{
	...
	uint32 TimeTotalTime = sConfigMgr->GetIntDefault("Unell.Top.TotalTime.Time.Reset", 3600);
...
void InitTopKill()
{
    time_t curtime = time(NULL);
	uint32 time32 = curtime;
	uint32 TimeTotalKill = sConfigMgr->GetIntDefault("Unell.Top.TotalKill.Time.Reset", 3600);
...
public:
	...
	{		
		if (sConfigMgr->GetBoolDefault("Unell.Top.Enable", true))
		{
			if (TimeChecker < diff)
			{
				uint32 CheckTime = sConfigMgr->GetIntDefault("Unell.Top.CheckTime", 60000);
