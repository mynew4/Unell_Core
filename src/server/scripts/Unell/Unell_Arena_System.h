...
static bool Arena1v1CheckTalents(Player *player)
{
    ...
    if (sConfigMgr->GetBoolDefault("Arena.1v1.BlockForbiddenTalents", true) == false)
        return true;
    ...
    if (count >= 36)
    {
        handler.PSendSysMessage("You can't join, because you have invested too many points in a forbidden talent. Please edit your talents.");
        return false;
    }
    else
        return true;
