...
public:
    ...
    static bool HandleServerInfoCommand(ChatHandler* handler, char const* /*args*/)
    {
		...
		handler->PSendSysMessage("|cFFFF0000#|cff6C8CD5 Ядро сервера: |cFFFF0000Unell Core rev.|r %s %s", _HASH, _DATE);		
		handler->PSendSysMessage("|cFFFF0000# |cff6C8CD5Онлайн игроков: |cff14ECCF%u |cff6C8CD5(максимум: |cff14ECCF%u|cff6C8CD5)", playerCount, connPeak);
		handler->PSendSysMessage(LANG_UPTIME, uptime.c_str());
		
		if (latency == 0)
		{
			handler->PSendSysMessage("|cFFFF0000#|cff6C8CD5 Задержка: |cff14ECCFНеопределенно");
		}
		else
			handler->PSendSysMessage("|cFFFF0000#|cff6C8CD5 Задержка: |cff14ECCF%u мс", latency);
