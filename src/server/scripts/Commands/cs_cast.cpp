...
public:
    ...
    static bool HandleCastCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (handler->GetSession()->GetSecurity() < SEC_DEVELOPER && (bounds.first != bounds.second || spellDifficultyId || spellInfo->HasEffect(SPELL_EFFECT_SKILL_STEP) || spellInfo->HasEffect(SPELL_EFFECT_TRADE_SKILL)))
        {
