...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> banlistCommandTable =
        {
            ...
            { "character",      SEC_GAMEMASTER,  true,  &HandleBanListCharacterCommand_Unell,   "" },
    ...
    static bool HandleBanCharacterCommand(ChatHandler* handler, char const* args)
    {
		...
		std::string author = handler->GetSession() ? NameLink.c_str() : "Server";
        ...
        switch (sWorld->BanCharacter(name, durationStr, reasonStr, author))
        {
			...
			if (atoi(durationStr) > 0)
                {
                    if (sConfigMgr->GetBoolDefault("Unell.World.Announe.Ban", true))
    ...
    static bool HandleBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
		...
		Player* player = handler->GetSession()->GetPlayer();
		std::string NameLink = ChatHandler(player->GetSession()).GetNameLink(player);		
		std::string author = handler->GetSession() ? NameLink.c_str() : "System";
            ...
            case BAN_SUCCESS:
                switch (mode)
                {
					...
					case BAN_IP:
						...
						{
							if (sConfigMgr->GetBoolDefault("Unell.World.Announe.Ban", true))
								sWorld->SendWorldText(LANG_BAN_IP_YOUBANNEDMESSAGE_WORLD, author.c_str(), nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(durationStr), true).c_str(), reasonStr);
							else
								handler->PSendSysMessage(LANG_BAN_YOUBANNED, nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(durationStr), true).c_str(), reasonStr);
						}
						else
						{
							if (sConfigMgr->GetBoolDefault("Unell.World.Announe.Ban", true))
								sWorld->SendWorldText(LANG_BAN_IP_YOUPERMBANNEDMESSAGE_WORLD, author.c_str(), nameOrIP.c_str(), reasonStr);
							else
								handler->PSendSysMessage(LANG_BAN_YOUPERMBANNED, nameOrIP.c_str(), reasonStr);
						}
					break;
					case BAN_CHARACTER:
						...
						{
							if (sConfigMgr->GetBoolDefault("Unell.World.Announe.Ban", true))
								sWorld->SendWorldText(LANG_BAN_CHARACTER_YOUBANNEDMESSAGE_WORLD, author.c_str(), nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(durationStr), true).c_str(), reasonStr);
							else
								handler->PSendSysMessage(LANG_BAN_YOUBANNED, nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(durationStr), true).c_str(), reasonStr);
						}
						else
						{
							if (sConfigMgr->GetBoolDefault("Unell.World.Announe.Ban", true))
								sWorld->SendWorldText(LANG_BAN_CHARACTER_YOUPERMBANNEDMESSAGE_WORLD, author.c_str(), nameOrIP.c_str(), reasonStr);
							else
								handler->PSendSysMessage(LANG_BAN_YOUPERMBANNED, nameOrIP.c_str(), reasonStr);
						}
						break;
	...
	static bool HandleBanListCharacterCommand_Unell(ChatHandler* handler, char const* args)
	{
		char* filterStr = strtok((char*)args, " ");
		std::string filter = filterStr ? filterStr : "";		
		...
		if (filter.empty())
		{
			PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_BANNED_ALL);
			result = CharacterDatabase.Query(stmt);
		}
		else
		{
			PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(CHAR_SEL_GUID_BY_NAME_FILTER);
			stmt->setString(0, filter);
			result = CharacterDatabase.Query(stmt);
		}
		...
		return HandleBanListHelperChar(result, handler);
	}
	
	static bool HandleBanListHelperChar(PreparedQueryResult result, ChatHandler* handler)
	{
		handler->PSendSysMessage(LANG_BANLIST_MATCHINGACCOUNT);
		...
		if (handler->GetSession())
		{
			...
			{
				Field* fields = result->Fetch();
				uint32 charid = fields[0].GetUInt32();

				QueryResult banResult = CharacterDatabase.PQuery("SELECT characters.name FROM characters, character_banned WHERE character_banned.guid='%u' AND character_banned.guid=characters.guid", charid);
				if (banResult)
				{
					Field* fields2 = banResult->Fetch();
					handler->PSendSysMessage("%s", fields2[0].GetCString());
		...
		else
		{
			...
			do
			{
				...
				// "character" case, name need extract from another DB
					...
					AccountMgr::GetName(charid, accountName);

				// No SQL injection. id is uint32.
				QueryResult banInfo = CharacterDatabase.PQuery("SELECT bandate, unbandate, bannedby, banreason FROM character_banned WHERE guid = %u ORDER BY unbandate;", charid);
				if (banInfo)
				{
					Field* fields2 = banInfo->Fetch();
					do
