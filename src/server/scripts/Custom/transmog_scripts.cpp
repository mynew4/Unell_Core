...
public:
...
#endif
        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tУдалить все трансмогрификации", EQUIPMENT_SLOT_END + 2, 0, "Удалить все трансмогрификации со всех одетых предметов?", 0, false);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
    {
        ...
        switch (sender)
        {
            ...
            case EQUIPMENT_SLOT_END + 6: // view preset
            {
                ...
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|tUse set", EQUIPMENT_SLOT_END + 5, action, "Использование этого набора для трансформирования сделает предметы персональными.\nВы желаете продолжить?\n\n" + sT->presetByName[player->GetGUID()][action], 0, false);
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-LeaveItem-Opaque:30:30:-18:0|tУдалить сет", EQUIPMENT_SLOT_END + 7, action, "Are you sure you want to delete " + sT->presetByName[player->GetGUID()][action] + "?", 0, false);
            ...
            case EQUIPMENT_SLOT_END + 8: // Save preset
            {
                ...
                if (canSave)
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|tСохранить сет", 0, 0, "Введите название сета", cost*sT->GetSetCostModifier() + sT->GetSetCopperCost(), true);
    ...
    void ShowTransmogItems(Player* player, Creature* creature, uint8 slot) // Only checks bags while can use an item from anywhere in inventory
    {
        ...
        if (oldItem)
        {
            ...
            for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
            {
                ...
                if (sT->GetFakeEntry(oldItem->GetGUID()) == newItem->GetEntry())
                    continue;
                ++limit;
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sT->GetItemLink(newItem, session), slot, newItem->GetGUIDLow(), "Использование этого набора для трансформирования сделает предметы персональными.\nВы желаете продолжить?\n\n" + sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sT->GetItemLink(newItem, session) + ss.str(), price, false);
            }
        ...
        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tУдаление трансмогрификации", EQUIPMENT_SLOT_END + 3, slot, "Удалить трансмогрификацию в данном слоте?", 0, false);
...
public:
    ...
    void OnBeforeConfigLoad(bool reload) override
    {
        if (!reload) 
		{
            ...
            std::string cfg_file = conf_path + "/modules/transmog.conf";			
