...
void AnticheatMgr::JumpHackDetection(Player* player, MovementInfo /* movementInfo */, uint32 opcode)
{
	...
	if (m_Players[key].GetLastOpcode() == MSG_MOVE_JUMP && opcode == MSG_MOVE_JUMP)
	{
		...
		sLog->outAntiCheat("Jump-Hack detected player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
	}
}
...
void AnticheatMgr::WalkOnWaterHackDetection(Player* player, MovementInfo  movementInfo)
{
	...
	if (sConfigMgr->GetBoolDefault("Anticheat.KickPlayerWaterWalkHack", false))
	{
		...
		sLog->outAntiCheat("Walk on Water - Hack detected and counteracted by kicking player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
	}
	else 
	{
		sLog->outString("AnticheatMgr:: Walk on Water - Hack detected player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
		sLog->outAntiCheat("Walk on Water - Hack detected player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
	}
...
void AnticheatMgr::FlyHackDetection(Player* player, MovementInfo  movementInfo)
{
	...
	if (sConfigMgr->GetBoolDefault("Anticheat.KickPlayerFlyHack", false))
	{
		...
		sLog->outAntiCheat("Fly-Hack detected and counteracted by kicking player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
		player->GetSession()->KickPlayer(true);
	}
	else
	{
		sLog->outString("AnticheatMgr:: Fly-Hack detected player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
		sLog->outAntiCheat("Fly-Hack detected player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
	}    
...
void AnticheatMgr::TeleportPlaneHackDetection(Player* player, MovementInfo movementInfo)
{
	...
	if (z_diff > 1.0f)
	{
		...
		sLog->outAntiCheat("Teleport To Plane - Hack detected player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
...
void AnticheatMgr::ClimbHackDetection(Player *player, MovementInfo movementInfo, uint32 opcode)
{
	...
	if (angle > CLIMB_ANGLE)
	{
		...
		sLog->outAntiCheat("Climb-Hack detected player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
...
void AnticheatMgr::SpeedHackDetection(Player* player, MovementInfo movementInfo)
{
	...
	{
		if (sConfigMgr->GetBoolDefault("Anticheat.KickPlayerSpeedHack", false))
		{
			player->GetSession()->KickPlayer(true);
			sLog->outString("AnticheatMgr:: Speed-Hack detected and kicking player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
			sLog->outAntiCheat("Speed-Hack detected and kicking player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
		}
		else
		{
			sLog->outString("AnticheatMgr:: Speed-Hack detected player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
			sLog->outAntiCheat("Speed-Hack detected player %s (%u)", player->GetName().c_str(), player->GetGUIDLow());
		}
...
void AnticheatMgr::AnticheatGlobalCommand(ChatHandler* handler)
{
	...
	if (!resultDB)
	{
		handler->PSendSysMessage("Таких игроков нет.");
		return;
	}
	else
	{
		handler->SendSysMessage("=============================");
		handler->PSendSysMessage("Игроки с самыми низкими средними значениями:");
		do
		{
			...
			if (Player* player = sObjectMgr->GetPlayerByLowGUID(guid))
				handler->PSendSysMessage("Игрок: %s В среднем: %f Всего репортов: %u", player->GetName().c_str(), average, total_reports);
	...
	else
	{
		...
		handler->PSendSysMessage("Игроки с наибольшим количеством репортов:");
		do
		{
			Field *fieldsDB = resultDB->Fetch();
			...
			if (Player* player = sObjectMgr->GetPlayerByLowGUID(guid))
				handler->PSendSysMessage("Игрок: %s Всего репортов: %u В среднем: %f", player->GetName().c_str(), total_reports, average);
