...
public:
    ...
    static bool HandleAntiCheatPlayerCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!guid)
        {
            handler->PSendSysMessage("Нет такого игрока.");
            return true;
        }
        ...
        handler->PSendSysMessage("Информация о игроке %s",player->GetName().c_str());
        handler->PSendSysMessage("В среднем: %f || Всего репортов: %u ",average,total_reports);
        handler->PSendSysMessage("СХ: %u || ФХ: %u || Джамп репорты: %u ",speed_reports,fly_reports,jump_reports);
        handler->PSendSysMessage("Бег по воде: %u  || Teleport To Plane репорты: %u",waterwalk_reports,teleportplane_reports);
        handler->PSendSysMessage("Climb репорты: %u", climb_reports);
