...
WorldSession::WorldSession(uint32 id, WorldSocket* sock, AccountTypes sec, bool ispremium, uint8 expansion, time_t mute_time, LocaleConstant locale, uint32 recruiter, bool isARecruiter, bool skipQueue):
...
_security(sec), _ispremium(ispremium), _skipQueue(skipQueue), _accountId(id), m_expansion(expansion), _logoutTime(0),
...
m_latency(0), m_clientTimeDelay(0), m_TutorialsChanged(false), recruiterId(recruiter), isRecruiter(isARecruiter), m_currentBankerGUID(0), m_currentVendorEntry(0), timeWhoCommandAllowed(0)
{
