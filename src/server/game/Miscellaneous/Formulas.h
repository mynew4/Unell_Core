    ...
    namespace XP
    {
        ...
        inline uint32 Gain(Player* player, Unit* u, bool isBattleGround = false)
        {
                ...
                !(creature->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_XP_AT_KILL)))
            {
				...
				{
					QueryResult db_level = CharacterDatabase.PQuery("SELECT level FROM `guild_level` WHERE `guild` = '%u'", guildid);
					if (db_level)
					{
						uint32 level = db_level->Fetch()->GetInt32();
						QueryResult db_reward = CharacterDatabase.PQuery("SELECT rate_xp FROM guild_level_reward WHERE `level` = '%u'", level);

						if (db_reward)
						{
							float rate_xp_guild = db_reward->Fetch()->GetFloat();

							if (rate_xp_guild > 0.01f)
								xpMod *= rate_xp_guild;
						}
					}
				}
