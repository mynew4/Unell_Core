...
#define MIN_UNELL_STRING_ID           1                    // 'unell_string'
#define MAX_UNELL_STRING_ID           2000000000
...
class ObjectMgr
{
    ...
    public:
		...
		// Unell AntiAD
        void LoadUnellADNames();
        bool IsADName(std::string const& name) const;
		...
		// Unell String
		static void AddUnellLocaleString(std::string const& s, LocaleConstant locale, StringVector& data);
		static inline void GetUnellLocaleString(const StringVector& data, int loc_idx, std::string& value)
		{
			if (data.size() > size_t(loc_idx) && !data[loc_idx].empty())
				value = data[loc_idx];
		}
