...
int Master::Run()
{
    ...
    sLog->outString("Нажмите <Ctrl-C> чтобы остановить.\n");
    ...
    if (highPriority)
    {
        if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
            ...
            sLog->outError("Невозможно присвоить процессу высокий приоритет");
    }
    ...
    if (highPriority)
    {
        if (setpriority(PRIO_PROCESS, 0, PROCESS_HIGH_PRIORITY))
            ...
            sLog->outString("Процессу присвоен приоритет класса %i", getpriority(PRIO_PROCESS, 0));
    }
    ...
    sLog->outString("%s (worldserver-daemon) загружен...", _FULLVERSION);
...
bool Master::_StartDB()
{
	...
	// Unell Database
	///- Get Unell database info from configuration file
    dbstring = sConfigMgr->GetStringDefault("UnellDatabaseInfo", "");
    if (dbstring.empty())
    {
        sLog->outError("Unell database not specified in configuration file");
        return false;
    }

    async_threads = uint8(sConfigMgr->GetIntDefault("UnellDatabase.WorkerThreads", 1));
    if (async_threads < 1 || async_threads > 32)
    {
        sLog->outError("Unell database: invalid number of worker threads specified. "
            "Please pick a value between 1 and 32.");
        return false;
    }

    synch_threads = uint8(sConfigMgr->GetIntDefault("UnellDatabase.SynchThreads", 1));
    ///- Initialise the Unell database
    if (!UnellDatabase.Open(dbstring, async_threads, synch_threads))
    {
        sLog->outError("Cannot connect to unell database %s", dbstring.c_str());
        return false;
    }
